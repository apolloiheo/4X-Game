...
public class WorldGenerator : MonoBehaviour
{
    ...
    void DetermineContinents(World _world)
    {
        int worldLength = _world.GetLength();
        int worldHeight = _world.GetHeight();
    }

    /* Determine the area of Continents proportional to world size. */
    void DetermineLand(World _world)
    {
        // Different Procedures given different numbers of continents
        int totalWorldSize = _world.GetLength() * _world.GetHeight();
        ...
        {
            case 1: // Fractal Map
                int StartX = _world.GetLength()/2;
                int StartY = _world.GetHeight()/2;
                int numWalkers = 5; //creating this many walkers
                WorldGenWalker[] walkers = new WorldGenWalker[numWalkers]; //see WorldGenWalker class
                GameTile startTile = _world.GetTile(StartX, StartY); //start tile for walkers is just center of map
                //startTile.SetTerrain(2);
                
                for (int i = 0; i < numWalkers; i++)
                {
                    walkers[i] = new WorldGenWalker(_world, startTile,"biome", 7, 1, _random); //fills list with walkers
                }
                ...
                while (currentWorldCoverage < desiredWorldCoverage)
                {
                    foreach (WorldGenWalker walker in walkers) //goes through all the walkers in the list
                    {
                        ...
                        {
                            if (walker._currTile.GetYPos() > _world.GetHeight() * 0.7) //being above 70% of world height is too high
                            {
                            ...
                            if (walker._currTile.GetYPos() < _world.GetHeight() * 0.3) //being above 30% of world height is too high
                            {
            ...
            case 2: // Two Standard Continents
                ...
                int continentStartXWest = _random.NextInt((int)(_world.GetLength()  * .25), (int)(_world.GetLength() * .35));
                int continentStartYWest = _random.NextInt((int)(_world.GetHeight() * .25), (int)(_world.GetHeight() * .75));
                int continentStartXEast = _random.NextInt((int)(_world.GetLength() * .65), (int)(_world.GetLength() * .75));
                int continentStartYEast = _random.NextInt((int)(_world.GetHeight() * .25), (int)(_world.GetHeight() * .75));
                ...
                foreach (GameTile t in _world.GetTile(continentStart1).GetNeighbors())
                {
                    queue.Enqueue(new Point(t.GetXPos(), t.GetYPos()));
                }
                
                // Turn both continent starting points to land.
                _world.ModifyTileBiome(continentStart1, 1);
                _world.ModifyTileBiome(continentStart2, 1);
                ...
                while (currentWorldCoverage < desiredWorldCoverage)
                {
                    ...
                    if (currentWorldCoverage >= desiredWorldCoverage * continentSwitch && !continentSwitched)
                    {
                        ...
                        foreach (GameTile t in _world.GetTile(continentStart2).GetNeighbors())
                        {
                            queue.Enqueue(new Point(t.GetXPos(), t.GetYPos()));
                        }
                    }
                    ...
                    GameTile currentGameTile = _world.GetTile(queue.Dequeue());
                    ...
                    while (possibleNeighbors.Count > 0)
                    {
                        ...
                        if (currentNeighbor.GetYPos() < _world.GetHeight() / 2)
                        {
                            if (currentNeighbor.GetYPos() < _world.GetHeight() * .15)
                            {
                                divisionFactor = 1;
                            }
                            heightFactor = (_world.GetHeight() / 2) - currentNeighbor.GetYPos() / divisionFactor;
                        }
                        else
                        {
                            if (currentNeighbor.GetYPos() > _world.GetHeight() * .85)
                            {
                                divisionFactor = 1;
                            }
                            heightFactor = (currentNeighbor.GetYPos() - (_world.GetHeight() / 2)) / divisionFactor;
                        }
                        
                        // Increases the closer currentNeighbor's X is to the center X of the world length.
                        int distanceToCenterFactor; 
                        
                        if (currentNeighbor.GetXPos() < _world.GetLength() / 2)
                        {
                            if (currentNeighbor.GetXPos() > _world.GetLength() * .40)
                            {
                                divisionFactor = 1;
                            }
                        ...
                        {
                            if (currentNeighbor.GetXPos() < _world.GetLength() * .60)
                            {
                                divisionFactor = 1;
                            }
                            distanceToCenterFactor = ((_world.GetLength() - (currentNeighbor.GetXPos())) / divisionFactor);
                        }
                        // Increases the closer currentNeighbor's X is to 0 or to max world.Length
                        int distanceToEdgeFactor; 
                        if (currentNeighbor.GetXPos() < _world.GetLength() / 2)
                        {
                            if (currentNeighbor.GetXPos() < _world.GetLength() * .15)
                            {
                                divisionFactor = 1;
                            }
                            distanceToEdgeFactor = ((_world.GetLength() / 2) - currentNeighbor.GetXPos()) / divisionFactor;
                        }
                        else
                        {
                            if (currentNeighbor.GetXPos() > _world.GetLength() * .85)
                            {
                                divisionFactor = 1;
                            }
                            distanceToEdgeFactor = (currentNeighbor.GetXPos() - (_world.GetLength() / 2)) / divisionFactor;
                        }
                        ...
                        if (probability > probabilityThreshold + heightFactor + distanceToCenterFactor + distanceToEdgeFactor - (consecutiveFailures * failureFactor))
                        {
                            ...
                            if (_world.GetTile(neighborLocation).GetBiome() != 0)
                            {
                                // Modify the Tile's Biome 
                                _world.ModifyTileBiome(neighborLocation, 1);
                            }
    ...
    void DetermineTerrain(World _world)
    {
        // Determine Mountain Ranges
        int randomX = _random.NextInt(_world.GetLength()/4, _world.GetLength() * 3/4);
        int randomY = _random.NextInt(0, _world.GetHeight());
        ...
        {
            randomX = _random.NextInt(_world.GetLength()/4, _world.GetLength() * 3/4);
            randomY = _random.NextInt(0, _world.GetHeight());
            walkers[i] = new WorldGenWalker(_world, _world.GetTile(randomX, randomY), "terrain", 0, 2, _random);
    ...
    void DetermineRiversAndLakes(World _world)
    {
        ...
        AssignRemainingFreshWaterAccess(_world);
    ...
    void DetermineFeatures(World _world)
    {
        DetermineWoods();
            ...
            for (int x = 0; x < _world.GetLength(); x++)
            {
                for (int y = 0; y < _world.GetHeight(); y++)
                {
                    ...
                    if (currTile.GetBiome() == 1 && currTile.GetTerrain() != 2 && !currTile.GetRiverAdjacency())
                    {
                        ...
                        if (y < _world.GetHeight() * .525 && y > _world.GetHeight() * .475)
                        {
                            // 80% Chance
                            if (probability < 80)
                            {
                        ...
                        else
                        {
                            ...
                            int distanceToEquator = Math.Abs(y - (_world.GetHeight() / 2));
    ...
    void DetermineResources(World _world)
    {
        
    }
    
    /* Determines Spawn Points */
    void DetermineSpawnPoints(World _world, int civCount)
    {
        ...
        for (int x = 0; x < _world.GetLength(); x++) //looking through all tiles near middle vertically
        {
        ...
        int randomNum = _random.NextInt(0, possiblePoints.Count); 
        ...
        _world.spawnPoints = spawnPointList;
    }
}
